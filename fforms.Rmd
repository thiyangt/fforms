...
We generated several initial plots to visualise the vote matrix calculations based on OOB observations to get a better understanding of the relationships learned by the random forest. Out of different plots we generated "heatmap" seems to be the most useful in visualising vote matrix probabilities. We use iheatmapr function in iheatmapr package in R to visualise interactive heatmap with clustering and row and column annotations. The ordering of the rows and columns are based on hierarchical clustering approach. The visualizations are shown in 

Visualising the data in this manner helps us to identify several important characteristics learned by the ranom forest algorithm 

**Yearly time series**,

- Irrespective of the true classlable, the random walk with drift has been assigned a non-zero probability by all of the yearly series. This means, for all of the yearly series at least one tree in the random forest voted for random walk with drift. According to the results in \autoref{forecasts} also we can see random walk with drift perform extremely well across all time series. Furthermore, the majority of yearly series in  M4comp are highly trended.

- For all yearly series, ETS with damped trend has been assinged a very low vote probability.


- There is a matching correspondence between the vote probability matrix and the composition of clusters by true class labels. For example, in cluster 1 and cluster 2 the majority of time series are labelled as random walk with drift (rwd), ARIMA and ETS with trend (ETS-trend). In the vote probability matrix, high probability varules have been assigned to those three classes. Furthermore, in cluster 3 the majority of time series are labelled as rwd, and the series in cluster 3 have selected random walk with drift with high vote probability. In cluster 4, the majority of series are in rwd and ETS-trend and they have been assigned a very high vote probability. In cluster 5, neural network (nn), white noise(wn) and rwd have been assigned a very high probability and the majority of time series in cluster 5 are classified as nn, wn or rwd. This indicates that our meta-learner correctly predicts the classlabels. 

- Some more intricate insights also emerged when zooming in on particular sections of the vote matrix probabilities. For example in cluster 1, the series that are labelled as ARIMA, have also selected ETS-trend with very high probability.
Furthermore, in cluster 5, series that are labeled as "wn" have selected  "wn" as their predicted class label with high vote probability, and they also assigned relatively large probability for ARMA/MA/AR while a very small probability has been assigned to ETS-trend and ARIMA. This indicates our meta-learner correctly ientified the simmilarities and dissimilarities between classes.

- According to the feature distribution we can see the most of the series in clusters 1-3 are not difference stationary while the series in clusters 4-5 are difference stationary. That could be the one reason both ARIMA and ETS-trend have been assigned a very high probability in clusters 4 and 5 compared to other. This indicates our random forest correctly identify the relationship between class labels and features. 

- Based on the  vote probability similarities, the columns of the matrix are classified into four clusters as 
  i) Cluster 1 : nn, rw, theta
  ii) Cluster 2: ARMA/AR/MA, ETS-notrend, ETS-damped trend, ETS-notrendnoseasonal, wn
  iii) CLuster 3: rwd
  iv) Cluster 4: ARIMA, ETS-trend
\end{item}
  
  Cluster 1 models are general models that can be applied to any series, Cluster 2 models are suitable for less trended series, CLuster 3 is random walk with drift on its own, Cluster 4 models are suitable for highly trended series. 


**Monthly series**

- The vote matrix columns are divided into four cluster according as 
    i) Cluster 1: SARIMA, tbats, theta
    ii) Cluster 2: nn, rw, stlar
    iii) Cluster 3: ARIMA, ARMA/AR/MA, ETS-dampedtrend, ETS-notrendnoseasonal, ETS-trend, rw, wn
    iv) Cluster 4: ETS-dampedtrendseasonal, ETS-seasonal, ETS-trendseasonal, snaive
    
Cluster 1 models are suitable to model time series with high seasonality, Cluster 2 models canbe used to model time series with complicated behaviour (series with high entropy), Cluster 3 models are non-seasonal time series modes, Cluster 4 models are ETS models with trend component and snaive. Hence, we can say monthly meta-learner correctly identify the similarities and dissimilaries between the classes.

- In general, SARIMA, tbats, theta, nn, rwd and stlr have been assigned a very high vote probability. 

- For all monthly series, ARMA/MA/AR  has a very low probability.

- Some series have selected stlar, nn and SARIMA models with very high vote probability. 

- Row-wise clustering classifies time series into 5 main clusters as follows

  Cluster 1: stlar has been asigned a very high vote probability
  
  Cluster 2: most of the series in this cluster have selected rwd or stlar as their predicted class label. 
  
  Cluster 3: series in this cluster select nn with high vote probability
  
  Cluster 4: Some series in this cluster select non-seasonal time series models (AR, MA, ARMA, wn, rw, ETS models with out seasonal component) with high probability. The reason could be, according to the feature distribution, the strength of seasonality of the time series in this cluster are very low.
  
  Cluster 5: Most of the series select SARIMA or stlar with high probability. According to the feature distribution of strength of seasonality, the series in this cluster are highly seasonal. 

Overall, the above findings of clusters consists with the composition of clusters by true class labels shown in Figure : panel A. 

According to xx, the vote matrix visualisations of quarterly data depicted a similar pattern to monthly data. For quarterly and monthly data, the same set of features and class labels are used to train the model, Hence, this consistency between the results of the quarterly and the monthly series would provide evidence in support of the validity and trustability of the meta-learning framework.
...
- A very low vote proabilities have been assigned to rw, rwd, theta and wn. 

- For most of the series nn has been assigned a non-zero vote probability.

- Row-wise clustering classifies time series into 5 main clusters as follows

cluster 1: All time series in cluster 1 have selected nn with very high probability. 

Cluster 2: Most of the time series selects stlar with high probability.

Cluster 3: Most of the time series selects mstlarima with very high vote probability.

Cluster 4: Some series in the cluster select snaive while other series select tbats instead. Zooming out the results we can see the series that have selected snaive with hogh probability have assigned a very low probability for tbats. Further, exploration with the interactive dashboard it was clear series with high daily seasonality select snaive while series with high weekly seasonality select tbats approach.  


CLuster 5: Most of the series selects mstlets with very high probability. TZooming out the results we can see some of those series also assigned a high probability to mstlarima class as well.
...
```{r heaty, fig.height=10, fig.width=20, fig.cap="Visualization of vote matrix of yearly series. The X-axis denotes the class labels.", fig.pos="h"}
...
```{r classdisyearly, comment=NA, fig.width=15, fig.height=12}
...
```{r, fig.height=10, fig.width=20, fig.cap="Visualization of vote matrix of monthly series. The X-axis denotes the class labels.", fig.pos="h"}
...
```{r classlabledismonthly, comment=NA, fig.width=15, fig.height=12}
...
```{r heathourly, fig.height=10, fig.width=20, fig.cap="Visualization of vote matrix of hourly series. The X-axis denotes the class labels.", fig.pos="h"}
...
```{r hourlylabel, comment=NA, fig.width=15, fig.height=12}
...
```{r pca, comment=NA, fig.height=8, dev=c('png')}
