...
Clearly, there is a need for a fast and scalable algorithm to automate the process of selecting models with the aim of forecasting. \textcolor{red}{However, for a given time series the selection of an appropriate forecast model from many possibilities is not straightforward. This selection presents a challenge because each method performs best for some but not all tasks. The features of a time series are considered an important factor in identifying suitable forecasting models} [@collopy1992rule; @meade2000evidence; @makridakis2000m3; @wang2009rule].





We refer to this process as forecast model selection. \textcolor{red}{We propose a general meta-learning framework using features of the time series to select the class of models, or even the specific model, to be used for forecasting. The forecast model selection process
...
- Some more intricate insights also emerged when zooming in on particular sections of the vote matrix probabilities. For example in cluster 1, the series that are labelled as ARIMA, have also selected ETS-trend with very high probability. Furthermore, in cluster 5, series that are labeled as "wn" have selected  "wn" as their predicted class label with high vote probability, and they also assigned relatively large probability for ARMA/MA/AR while a very small probability has been assigned to ETS-trend and ARIMA. This indicates our meta-learner correctly ientified the simmilarities and dissimilarities between classes.


- Based on the  vote probability similarities, the columns of the matrix are classified into four clusters as: i) Cluster 1 : nn, rw, theta, ii) Cluster 2: ARMA/AR/MA, ETS-notrend, ETS-damped trend, ETS-notrendnoseasonal, wn, iii) CLuster 3: rwd, iv) Cluster 4: ARIMA, ETS-trend.
In general, Cluster 1 models are general models that can be applied to any series, Cluster 2 models are suitable for less trended series, Cluster 3 is random walk with drift on its own, Cluster 4 models are suitable for highly trended series. 
...
- The vote matrix columns are divided into four cluster according as: i) Cluster 1: SARIMA, tbats, theta; ii) Cluster 2: nn, rw, stlar; iii) Cluster 3: ARIMA, ARMA/AR/MA, ETS-dampedtrend, ETS-notrendnoseasonal, ETS-trend, rw, wn;  iv) Cluster 4: ETS-dampedtrendseasonal, ETS-seasonal, ETS-trendseasonal, snaive. Cluster 1 models are suitable to model time series with high seasonality, Cluster 2 models canbe used to model time series with complicated behaviour (series with high entropy), Cluster 3 models are non-seasonal time series modes, Cluster 4 models are ETS models with trend component and snaive. Hence, we can say monthly meta-learner correctly identify the similarities and dissimilaries between the classes.
...
- Row-wise clustering classifies time series into 5 main clusters as follows: i)cluster 1: All time series in cluster 1 have selected nn with very high probability, ii) Cluster 2: Most of the time series selects stlar with high probability; iii) Cluster 3: Most of the time series selects mstlarima with very high vote probability, iv) Cluster 4: Some series in the cluster select snaive while other series select tbats instead. Zooming out the results we can see the series that have selected snaive with hogh probability have assigned a very low probability for tbats. Further, exploration with the interactive dashboard it was clear series with high daily seasonality select snaive while series with high weekly seasonality select tbats approach.  
